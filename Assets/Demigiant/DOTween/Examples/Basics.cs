...
public class Basics : MonoBehaviour
{
	...
	void Start()
	{
		...
		// cubeA > SHORTCUTS WAj
	    cubeA.DOMove(new Vector3(-3.25f, 2, 0), 1).OnComplete(Done);//.SetRelative().SetLoops(-1, LoopType.Yoyo);

		// cubeB > GENERIC WAY
		DOTween.To(()=> cubeB.position, x=> cubeB.position = x, new Vector3(-2, 2, 0), 1).SetLoops(-1,LoopType.Yoyo);
    ...
    private void Done()
    {
        ...
        Debug.Log(" I moved it back");
        cubeA.DOMove(new Vector3(-1.25f, 0, 0), 1).SetEase(Ease.Linear);
    }
}